...
- (void)setup
{
    ...
    dispatch_async(sessionQueue, ^{
        ...
        if([weakSelf.session canSetSessionPreset:AVCaptureSessionPreset640x480]){
            [weakSelf.session setSessionPreset:AVCaptureSessionPreset640x480];
        }
        ...
        if ([weakSelf.session canAddOutput:self.videoDataOutput]){
            [weakSelf.session addOutput:self.videoDataOutput];
   
            weakSelf.videoDataOutputQueue = dispatch_queue_create("videoDataOutput", NULL);
            [weakSelf.videoDataOutput setSampleBufferDelegate:self queue:self.videoDataOutputQueue];
        }
    ...
    static SystemSoundID soundID  =  0; if (soundID  ==  0) {
         ...
         NSString *path  =    [bundle pathForResource:@"LightNotice" ofType:@"mp3"];
...
-(void)setIsReOpnEye:(BOOL)isReOpnEye{
    _isReOpnEye = isReOpnEye;
}
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    if (self.isReOpnEye) {
        self.videoDataOutput.videoSettings = [NSDictionary dictionaryWithObjectsAndKeys:
                                          ...
                                          nil];
        [self setVideoDataOutput: self.videoDataOutput];
    }else{
        //获取灰度图像数据
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(onOutputFaceImage: faceImageFromSampleBuffer:)]){
        IFlyFaceImage* faceImage=[self faceImageFromSampleBuffer:sampleBuffer];
        [self.delegate onOutputFaceImage:faceImage faceImageFromSampleBuffer:sampleBuffer];
        faceImage=nil;
    }
}
